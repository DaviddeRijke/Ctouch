...
namespace Shop
{
    public class SR_RenderCamera : MonoBehaviour
    {
        ...
        public void Capture()
        {
            ...
            File.WriteAllBytes("Assets/Resources/FishModels/Icons/" + GetActiveFish()+ ".png", bytes);
        ...
        private string GetActiveFish()
        {
            for (int i = 0; i < contents.childCount; i++)
            {
                ...
                if (r.activeSelf) return r.name.ToLower()
                    .Substring(0, r.name.IndexOf("_prefab", StringComparison.Ordinal));
            }
            return "";
        }
    }
